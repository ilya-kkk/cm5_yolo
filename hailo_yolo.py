#!/usr/bin/env python3
"""
Direct Hailo YOLO processing using Python API
Bypasses HailoRT CLI issues and works directly with the device
"""
...
class HailoYOLOProcessor:
    ...
    def init_hailo(self):
        ...
        try:
            ...
            # Try to get Hailo device directly
            try:
                # Try different ways to access Hailo device
                if hasattr(hailo_platform, 'HailoDevice'):
                    ...
                    if devices:
                        ...
                        print("⚠️ No Hailo devices found via HailoDevice.scan()")
                else:
            ...
            if self.hailo_device:
                try:
                    ...
                    if hef_path:
                        ...
                        # Try to load model (implementation depends on Hailo API version)
